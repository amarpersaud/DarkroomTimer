const StepDurationType=Object.freeze({SECONDS:"seconds",TIMES:"times",INVERSIONS:"inversions"});Object.freeze(StepDurationType);class DevelopmentStep{depth_limit=3;step_id=0;step_type="Chemical";step_name="DefaultStepName";step_duration=60;step_repeat_enabled=false;step_repeat_until_end=false;step_repeat_count=0;step_frequency=30;step_temperature=20;sub_steps=[];step_parent=-1;depth=0;constructor(id,type,name,duration,repeat_enabled,repeat_until_end,repeat_count,frequency,temperature,sub_steps,depth,parent_id){this.step_id=id;this.step_type=type;this.step_name=name;this.step_duration=duration;this.step_repeat_enabled=repeat_enabled;this.step_repeat_until_end=repeat_until_end;this.step_repeat_count=repeat_count;this.step_frequency=frequency;this.step_temperature=temperature;this.sub_steps=sub_steps;this.depth=depth;this.step_parent=parent_id;}
...
var m_RootStep=new DevelopmentStep(-1,"","",0,false,false,0,0,20,[],0,-1);var CurrentProfile=new DevelopmentProfile(m_RootStep,"Unnamed Profile 2",0,0);function index_of_step_id(step_id){for(let i=0;i<CurrentProfile.RootStep.sub_steps.length;i++){if(CurrentProfile.RootStep.sub_steps[i].step_id==step_id){return i;}}
return-1;}
...
function step_save_edit_clicked(){let itm=CurrentProfile.RootStep.find_element_by_id(CurrentProfile.current_editing_step_id);let new_step_name=document.getElementById("step_name_edit").value;let new_step_duration=document.getElementById("step_duration_edit").value;let new_step_type=document.getElementById("step_type_edit").value;let new_step_frequency=document.getElementById("step_frequency_edit").value;let new_step_repeat_count=document.getElementById("step_repeat_count_edit").value;let new_repeat_enabled=document.getElementById("step_repeat_edit").checked;let new_repeat_until_end=document.getElementById("step_repeat_until_end_edit").checked;let new_step_temp=document.getElementById("step_temperature_edit").value;itm.step_name=new_step_name;itm.step_duration=new_step_duration;itm.step_type=new_step_type;itm.step_frequency=new_step_frequency;itm.step_repeat_enabled=new_repeat_enabled;itm.step_repeat_until_end=new_repeat_until_end;itm.step_repeat_count=new_step_repeat_count;itm.step_temperature=new_step_temp;refreshUI();}
function step_add_edit_clicked(){let new_step_name=document.getElementById("step_name_edit").value;let new_step_duration=document.getElementById("step_duration_edit").value;let new_step_type=document.getElementById("step_type_edit").value;let new_step_frequency=document.getElementById("step_frequency_edit").value;let new_step_repeat_count=document.getElementById("step_repeat_count_edit").value;let new_repeat_enabled=document.getElementById("step_repeat_edit").checked;let new_repeat_until_end=document.getElementById("step_repeat_until_end_edit").checked;let new_step_temp=document.getElementById("step_temperature_edit").value;CurrentProfile.current_editing_step_id=CurrentProfile.current_step_id;add_step(new_step_name,new_step_type,new_step_duration,new_repeat_enabled,new_repeat_until_end,new_step_repeat_count,new_step_frequency,new_step_temp,[]);CurrentProfile.RecalculateStepID();}
...
function step_add_to_clicked(step_id){let itm=CurrentProfile.RootStep.find_element_by_id(step_id);if(itm!=null&&itm.depth<depth_limit){let new_step_name=document.getElementById("step_name_edit").value;let new_step_duration=document.getElementById("step_duration_edit").value;let new_step_type=document.getElementById("step_type_edit").value;let new_step_frequency=document.getElementById("step_frequency_edit").value;let new_step_repeat_count=document.getElementById("step_repeat_count_edit").value;let new_step_repeat_enabled=document.getElementById("step_repeat_edit").checked;let new_step_repeat_until_end=document.getElementById("step_repeat_until_end_edit").checked;let new_step_temp=document.getElementById("step_temperature_edit").value;let new_step=new DevelopmentStep(CurrentProfile.current_step_id,new_step_type,new_step_name,new_step_duration,new_step_repeat_enabled,new_step_repeat_until_end,new_step_repeat_count,new_step_frequency,new_step_temp,[],itm.depth+1,itm.step_id);CurrentProfile.current_editing_step_id=CurrentProfile.current_step_id;itm.sub_steps.push(new_step);CurrentProfile.RecalculateStepID();}
refreshUI();}
...
function add_step(step_name,step_type,step_duration,step_repeat_enabled,step_repeat_until_end,step_repeat_count,step_frequency,step_temperature,step_sub_steps){let new_step=new DevelopmentStep(CurrentProfile.current_step_id,step_type,step_name,step_duration,step_repeat_enabled,step_repeat_until_end,step_repeat_count,step_frequency,step_temperature,step_sub_steps,1,CurrentProfile.RootStep.step_id);CurrentProfile.RootStep.sub_steps.push(new_step);CurrentProfile.RecalculateStepID();refreshUI();}
...
function JsonLoadedStepToDevelopmentStep(JsonObj){if(JsonObj!=null){let newStep=new DevelopmentStep(JsonObj.step_id,JsonObj.step_type,JsonObj.step_name,JsonObj.step_duration,JsonObj.step_repeat_enabled,JsonObj.step_repeat_until_end,JsonObj.step_repeat_count,JsonObj.step_frequency,JsonObj.step_temperature,[],JsonObj.depth,JsonObj.step_parent);for(let i=0;i<JsonObj.sub_steps.length;i++){newStep.sub_steps.push(JsonLoadedStepToDevelopmentStep(JsonObj.sub_steps[i]));}
return newStep;}
return null;}
function JsonLoadedProfileToDevelopmentProfile(JsonObj){if(JsonObj!=null){let newRootStep=JsonLoadedStepToDevelopmentStep(JsonObj.RootStep);let newProfile=new DevelopmentProfile(newRootStep,JsonObj.ProfileName,JsonObj.PourInTime,JsonObj.PourOutTime);newProfile.current_editing_step_id=JsonObj.current_editing_step_id;console.log("New edit step id is "+newProfile.current_editing_step_id);return newProfile;}
return null;}
function UpdateRunProgress(percentage){let prog_bar_parent=document.getElementById("run_progress_bar");let prog_bar=prog_bar_parent.querySelector("progress");let prog_text=prog_bar_parent.querySelector("div");let newval=Math.floor(percentage*prog_bar.max);prog_bar.value=newval;prog_text.innerHTML=(percentage*100).toFixed(2);}
function RunProfile(){let maxTime=100.0;let numSteps=10000;for(let i=0;i<numSteps+1;i++){setTimeout(()=>{UpdateRunProgress(i/numSteps);},(i/numSteps)*maxTime*1000);}}