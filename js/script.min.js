...
function add_step(step_name,step_type,step_duration,step_repeat_enabled,step_repeat_until_end,step_frequency,step_temperature,step_sub_steps){let new_step=new DevelopmentStep(CurrentProfile.current_step_id,step_type,step_name,step_duration,step_repeat_enabled,step_repeat_until_end,step_frequency,step_temperature,step_sub_steps,1);new_step.step_parent=CurrentProfile.RootStep.step_id;CurrentProfile.current_step_id=CurrentProfile.current_step_id+1;CurrentProfile.RootStep.sub_steps.push(new_step);refreshUI();}
...
function build_step_HTML_recursive(itm,step_num){if(CurrentProfile.editing_enabled){let temp=document.getElementById("step_editing_template");let clon=temp.content.firstElementChild.cloneNode(true);if((itm.step_id==CurrentProfile.current_editing_step_id)&&CurrentProfile.editing_enabled){clon.setAttribute("class","step edited_step")}
...
return clon;}
else{let temp=document.getElementById("step_template");let clon=temp.content.firstElementChild.cloneNode(true);if((itm.step_id==CurrentProfile.current_editing_step_id)&&CurrentProfile.editing_enabled){clon.setAttribute("class","step edited_step")}
clon.querySelector(".step_num").innerHTML="[Step "+step_num.toString()+"] "+itm.step_name;clon.querySelector(".step_duration").innerHTML="Duration - "+itm.step_duration.toString();clon.querySelector(".step_temp").innerHTML="Temperature - "+itm.step_temperature.toString();let buttons=clon.querySelectorAll("button");buttons[0].setAttribute("onclick","step_edit_clicked("+itm.step_id+")");for(let i=0;i<itm.sub_steps.length;i++){let child=build_step_HTML_recursive(itm.sub_steps[i],i);clon.querySelector(".step_container").appendChild(child);}
return clon;}}
