...
get_max_step_id(){if(sub_steps.length==0){return this.step_id;}
let currentmax=0;for(let i=0;i<sub_steps.length;i++){if(sub_steps[i].step_id>currentmax){current_max=sub_steps[i].step_id;}}
return currentmax;}}
class DevelopmentProfile{RootStep=null;ProfileName="Unnamed Profile";PourInTime=0;PourOutTime=0;current_step_id=0;constructor(RootStep,ProfileName,PourInTime,PourOutTime){this.RootStep=RootStep;this.ProfileName=ProfileName;this.PourInTime=PourInTime;this.PourOutTime=PourOutTime;}
RecalculateStepID(step){let maxStepID=RootStep.get_max_step_id();this.current_step_id=maxStepID+1;}}
...
function delete_step(step_id){CurrentProfile.RootStep.remove_element_by_id(step_id);console.log("deleting step "+step_id.toString());refreshUI();}
function add_step(step_name,step_type,step_duration,step_repeat_enabled,step_repeat_until_end,step_frequency,step_temperature,step_sub_steps){let new_step=new DevelopmentStep(current_step_id,step_type,step_name,step_duration,step_repeat_enabled,step_repeat_until_end,step_frequency,step_temperature,step_sub_steps,1);new_step.step_parent=CurrentProfile.RootStep.step_id;current_step_id=current_step_id+1;CurrentProfile.RootStep.sub_steps.push(new_step);refreshUI();}
